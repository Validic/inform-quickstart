...
async function ensureUser() {
  ...
  console.log('Checking for existing Validic users...');
  ...
  process.env.VALIDIC_USER_ID = userData.uid;
  process.env.VALIDIC_ACCESS_TOKEN = userData.marketplace.token;
  process.env.VALIDIC_MARKETPLACE_URL = userData.marketplace.url;
}
...
async function ensureStream() {
  ...
  console.log('Checking for existing Validic streams...');
  ...
  if (existing) {
    process.env.VALIDIC_STREAM_ID = existing.id;
  ...
  process.env.VALIDIC_STREAM_ID = stream.id;
...
function requireProvisioned(req, res, next) {
  if (!process.env.VALIDIC_USER_ID) {
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    user_id: process.env.VALIDIC_USER_ID,
    stream_id: process.env.VALIDIC_STREAM_ID,
    provisioned: !!process.env.VALIDIC_USER_ID,
    // NOTE: marketplace_url embeds the user access token — treat as sensitive
    marketplace_url: process.env.VALIDIC_MARKETPLACE_URL || null,
  });
});
...
app.get('/api/users', async (req, res) => {
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      return res.status(response.status).json({ error: `Validic API error: ${response.status}` });
    }
...
app.get('/api/curl', (req, res) => {
  const { ORG_ID, ORG_TOKEN, VALIDIC_USER_ID, VALIDIC_STREAM_ID } = process.env;
  const userId = req.query.user_id || VALIDIC_USER_ID;
  ...
  res.json({
    ...
    stream_connect: `curl -N "${STREAM_URL}/streams/${VALIDIC_STREAM_ID}/connect?token=${ORG_TOKEN}"`,
...
app.get('/api/apps', requireProvisioned, async (req, res) => {
  const { ORG_ID, ORG_TOKEN, VALIDIC_USER_ID } = process.env;
  const userId = req.query.user_id || VALIDIC_USER_ID;
  try {
...
app.get('/api/stream/connect', requireProvisioned, (req, res) => {
  const { ORG_TOKEN, VALIDIC_STREAM_ID } = process.env;
  sseProxy(
    `${STREAM_URL}/streams/${VALIDIC_STREAM_ID}/connect?token=${ORG_TOKEN}`,
    req,
    res
  );
});
...
async function start() {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`User ID: ${process.env.VALIDIC_USER_ID}`);
    console.log(`Stream ID: ${process.env.VALIDIC_STREAM_ID}`);
  });
}
