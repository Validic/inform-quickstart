...
export default function InformDemo({ apiBaseUrl }: { apiBaseUrl?: string }) {
  ...
  const [config, setConfig] = useState<ConfigValues>(() => ({
    ...
    streamsUrl: envConfig?.streamsUrl || '',
      ...
      <header className="flex-shrink-0 glass-panel m-4 mb-0 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-gray-600 hover:border-blue-400 hover:text-blue-300 transition-colors"
            style={{ color: 'var(--text-muted)' }}
            title="Upload a new configuration file"
          >
