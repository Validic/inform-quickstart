...
function parseConfigFile(content: string): ParseResult {
  ...
  const token = values['ORG_TOKEN'] || values['NEXT_PUBLIC_VALIDIC_TOKEN'];
  const orgId = values['ORG_ID'] || values['NEXT_PUBLIC_VALIDIC_ORG_ID'];
  const apiUrl = values['API_URL'] || values['NEXT_PUBLIC_VALIDIC_API_URL'];
  const datagenUrl = values['DATAGEN_URL'] || values['NEXT_PUBLIC_VALIDIC_DATAGEN_URL'];

  if (token && orgId && apiUrl && datagenUrl) {
    return { config: { token, orgId, apiUrl, datagenUrl }, userIds };
  }
...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      const savedConfig = localStorage.getItem('validic-config');
      const savedUsers = localStorage.getItem('validic-users');
      ...
      try {
        ...
        if (response.success && response.data && response.data.length > 0) {
          ...
          if (fetchedUserIds.length > 0) {
            ...
            localStorage.setItem('validic-users', JSON.stringify(fetchedUserIds));
          }
        }
      } catch (error) {
  ...
  const setConfigFromFile = useCallback((content: string): ParseResult => {
    ...
    if (result.config) {
      ...
      localStorage.setItem('validic-config', JSON.stringify(result.config));
      localStorage.setItem('validic-users', JSON.stringify(result.userIds));
