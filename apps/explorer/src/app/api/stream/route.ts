...
export async function GET(request: NextRequest) {
  ...
  const streamsUrl = request.nextUrl.searchParams.get('streamsUrl');
  ...
  if (!streamsUrl) {
    return new Response(JSON.stringify({ error: 'Missing streamsUrl parameter' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }

  let targetUrl = `${streamsUrl.replace(/\/+$/, '')}/replay?token=${token}`;
  if (date) {
    targetUrl += `&date=${date}`;
  }
